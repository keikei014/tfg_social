...
private:
    ...
    //MAPIR
    bool disable_odometry;
    std::string laser_scan_topic, laser_frame_id, odom_topic, odom_frame_id;
...
StageNode::StageNode(int argc, char** argv, bool gui, const char* fname, bool use_model_names)
{
    ...
    //MAPIR
    localn.param<std::string>("laser_scan_topic", laser_scan_topic, "laser_scan" );
    localn.param<std::string>("laser_frame_id", laser_frame_id, "laser_stage" );
    localn.param<bool>("disable_odometry", disable_odometry, false );
    localn.param<std::string>("odom_topic", odom_topic, "odom" );
    localn.param<std::string>("odom_frame_id", odom_frame_id, "odom" );
...
StageNode::SubscribeModels()
{
    ...
    for (size_t r = 0; r < this->positionmodels.size(); r++)
    {
        ...
        if (!disable_odometry)
            new_robot->odom_pub = n_.advertise<nav_msgs::Odometry>(mapName(odom_topic.c_str(), r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10);
        ...
        for (size_t s = 0;  s < new_robot->lasermodels.size(); ++s)
        {
            if (new_robot->lasermodels.size() == 1)
                new_robot->laser_pubs.push_back(n_.advertise<sensor_msgs::LaserScan>(mapName(laser_scan_topic.c_str(), r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10));
            else
                new_robot->laser_pubs.push_back(n_.advertise<sensor_msgs::LaserScan>(mapName(laser_scan_topic.c_str(), r, s, static_cast<Stg::Model*>(new_robot->positionmodel)), 10));
...
StageNode::WorldCallback()
{
    ...
    for (size_t r = 0; r < this->robotmodels_.size(); ++r)
    {
        ...
        for (size_t s = 0; s < robotmodel->lasermodels.size(); ++s)
        {
            ...
            if( sensor.ranges.size() )
            {
                ...
                if (robotmodel->lasermodels.size() > 1)
                    msg.header.frame_id = mapName(laser_frame_id.c_str(), r, s, static_cast<Stg::Model*>(robotmodel->positionmodel));
                else
                    msg.header.frame_id = mapName(laser_frame_id.c_str(), r, static_cast<Stg::Model*>(robotmodel->positionmodel));
            ...
            if (robotmodel->lasermodels.size() > 1)
                tf.sendTransform(tf::StampedTransform(txLaser, sim_time,
                                                      ...
                                                      mapName(laser_frame_id.c_str(), r, s, static_cast<Stg::Model*>(robotmodel->positionmodel))));
            else
                tf.sendTransform(tf::StampedTransform(txLaser, sim_time,
                                                      mapName("base_link", r, static_cast<Stg::Model*>(robotmodel->positionmodel)),
                                                      mapName(laser_frame_id.c_str(), r, static_cast<Stg::Model*>(robotmodel->positionmodel))));
        }
        ...
        if (!disable_odometry)
        {
            ...
            //
            odom_msg.header.frame_id = mapName(odom_frame_id.c_str(), r, static_cast<Stg::Model*>(robotmodel->positionmodel));
            ...
            tf.sendTransform(tf::StampedTransform(txOdom, sim_time,
                                                  mapName(odom_frame_id.c_str(), r, static_cast<Stg::Model*>(robotmodel->positionmodel)),
                                                  mapName("base_footprint", r, static_cast<Stg::Model*>(robotmodel->positionmodel))));
        ...
        ground_truth_msg.header.frame_id = "/map";
        //ground_truth_msg.header.frame_id = mapName(odom_frame_id.c_str(), r, static_cast<Stg::Model*>(robotmodel->positionmodel));
